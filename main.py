...
x0_f3 = np.array([2.0, 2.0])

os.makedirs('artifacts', exist_ok=True)

def save_convergence_plot(func_values, title):
    for method, values in func_values.items():
        ...
        plt.title(f"{title} - {method} Convergence")
        plt.legend()
        plt.grid(True)
        plt.savefig(f'artifacts/{title.replace(" ", "_").lower()}_{method.replace(" ", "_").lower()}_convergence.png')
        plt.close()
...
for func, grad, hess, x0, title in [
    ...
    for method_name, method in methods.items():
        if method_name == "Adam Optimizer":
            x_min, values, steps = method(func, x0, grad, max_iters=8000)
        elif method_name == "Quasi-Newton (BFGS)" or method_name == "Gradient Descent":
    ...
    save_convergence_plot(func_values, title)